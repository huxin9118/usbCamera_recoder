...
public class UVCCamera {
    ...
    public static final int CTRL_AE_PRIORITY	= 0x00000004;	// D2:  Auto-Exposure Priority 自动曝光次序
    public static final int CTRL_AE_ABS			= 0x00000008;	// D3:  Exposure Time (Absolute) 曝光时间
    ...
    public static final int CTRL_FOCUS_AUTO		= 0x00020000;	// D17: Focus, Auto 自动对焦
    ...
    public static final int PU_BRIGHTNESS		= 0x80000001;	// D0: Brightness 亮度
    public static final int PU_CONTRAST			= 0x80000002;	// D1: Contrast 对比
    public static final int PU_HUE				= 0x80000004;	// D2: Hue 色调
    public static final int PU_SATURATION		= 0x80000008;	// D3: Saturation 饱和度
    public static final int PU_SHARPNESS		= 0x80000010;	// D4: Sharpness 锐度
    public static final int PU_GAMMA			= 0x80000020;	// D5: Gamma 伽马
    public static final int PU_WB_TEMP			= 0x80000040;	// D6: White Balance Temperature 白平衡
    ...
    public static final int PU_GAIN				= 0x80000200;	// D9: Gain 信号增益
    public static final int PU_POWER_LF			= 0x80000400;	// D10: Power Line Frequency
    public static final int PU_HUE_AUTO			= 0x80000800;	// D11: Hue, Auto 自动色调
    public static final int PU_WB_TEMP_AUTO		= 0x80001000;	// D12: White Balance Temperature, Auto 自动白平衡
    ...
    public static final int PU_CONTRAST_AUTO	= 0x80040000;	// D18: Contrast, Auto 自动对比度
...
//		while(result != 0);
     ...
     * @param texture
     */
    public synchronized void setPreviewTexture(final SurfaceTexture texture, final int rotate) {	// API >= 11
    	final Surface surface = new Surface(texture);	// XXX API >= 14
    	nativeSetPreviewDisplay(mNativePtr, surface, rotate);
    }
     ...
     * @param surface
     */
    public synchronized void setPreviewDisplay(final Surface surface, final int rotate) {
    	nativeSetPreviewDisplay(mNativePtr, surface, rotate);
    }
...
//================================================================================
	public synchronized void setAutoContrast(final boolean autoContrast) {
		if (mNativePtr != 0) {
			Log.i(TAG, "setAutoContrast: autoContrast = "+autoContrast);
			nativeSetAutoContrast(mNativePtr, autoContrast);
		}
	}
	...
	public synchronized boolean getAutoContrast() {
		boolean result = true;
		if (mNativePtr != 0) {
			result = nativeGetAutoContrast(mNativePtr) > 0;
			Log.i(TAG, "getAutoWhiteBlance: autoContrast = " + result);
		}
		return result;
	}

//================================================================================
	public synchronized void setAutoHue(final boolean autoHue) {
		if (mNativePtr != 0) {
			Log.i(TAG, "setAutoHue: autoHue = "+autoHue);
			nativeSetAutoHue(mNativePtr, autoHue);
		}
	}
	...
	public synchronized boolean getAutoHue() {
		boolean result = true;
		if (mNativePtr != 0) {
			result = nativeGetAutoHue(mNativePtr) > 0;
			Log.i(TAG, "getAutoWhiteBlance: autoHue = " + result);
		}
		return result;
	}
...
//================================================================================
	public synchronized void setAutoWhiteBlance(final boolean autoWhiteBlance) {
    	if (mNativePtr != 0) {
			Log.i(TAG, "setAutoWhiteBlance: autoWhiteBlance = "+autoWhiteBlance);
	...
	public synchronized boolean getAutoWhiteBlance() {
    	...
    	if (mNativePtr != 0) {
			...
			Log.i(TAG, "getAutoWhiteBlance: autoWhiteBlance = " + result);
		}
    	return result;
    }
...
//================================================================================
	...
	public synchronized void setBacklight(final int backlight) {
		if (mNativePtr != 0) {
			...
			if (range > 0)
				nativeSetBacklightComp(mNativePtr, (int)(backlight / 100.f * range) + mGainMin);
		}
	}
	...
	public synchronized int getBacklight(final int backlight_abs) {
		...
		if (mNativePtr != 0) {
			...
			final float range = Math.abs(mBacklightCompMax - mBacklightCompMin);
			if (range > 0) {
				result = (int)((backlight_abs - mGainMin) * 100.f / range);
			}
		}
		return result;
	}
