...
int UVCPreview::setPreviewDisplay(ANativeWindow *preview_window, int rotate) {
	ENTER();
	pthread_mutex_lock(&preview_mutex);
	{
		if (mPreviewWindow != preview_window) {
			...
			if (LIKELY(mPreviewWindow)) {
				requestRotate = rotate;
				if(requestRotate == 0 || requestRotate == 180){
					ANativeWindow_setBuffersGeometry(mPreviewWindow, frameWidth, frameHeight, previewFormat);
				}
				else if(requestRotate == 90 || requestRotate == 270){
...
uvc_error_t RBG565Rotate(uvc_frame_t *in, uvc_frame_t *out, int rotate){
	if (UNLIKELY(uvc_ensure_frame_size(out, in->width * in->height * PREVIEW_PIXEL_BYTES) < 0))
	...
	if(rotate == 0){
		memcpy(dst,src, in->width * in->height * PREVIEW_PIXEL_BYTES); 
	}
    else if(rotate == 90){
		int n = 0;
		int linesize = in->width*PREVIEW_PIXEL_BYTES; 
		int i; 
		int j; 
		for(j = 0; j < in->width; j++) {
			for(i = in->height; i > 0; i--) { 
				memcpy(&dst[n],&src[linesize*(i-1)+j*PREVIEW_PIXEL_BYTES],PREVIEW_PIXEL_BYTES); 
				n+=PREVIEW_PIXEL_BYTES; 
			} 
		}
    }
	else if(rotate == 180){
		...
		int j; 
		for(i = in->height; i > 0; i--) { 
			for(j = in->width; j > 0; j--) {
				memcpy(&dst[n],&src[linesize*(i-1)+(j-1)*PREVIEW_PIXEL_BYTES],PREVIEW_PIXEL_BYTES); 
				n+=PREVIEW_PIXEL_BYTES; 
			} 
		}
    }
		...
		for(j = in->width; j > 0; j--) {
			for(i = 0; i < in->height; i++) { 
				memcpy(&dst[n],&src[linesize*i+(j-1)*PREVIEW_PIXEL_BYTES],PREVIEW_PIXEL_BYTES); 
				n+=PREVIEW_PIXEL_BYTES; 
			} 
		}
    }
	...
	if (LIKELY(b)) {
		...
		if (convert_func) {
			...
			if LIKELY(converted) {
				...
				if (!b) {
					uvc_frame_t *rotated;
					rotated = get_frame(frame->width * frame->height * pixcelBytes);
					if LIKELY(rotated) {
						b = RBG565Rotate(converted, rotated, requestRotate);
						if (!b) {
							pthread_mutex_lock(&preview_mutex);
							copyToSurface(rotated, window);
							pthread_mutex_unlock(&preview_mutex);
						} else {
							LOGE("failed rotating");
						}
						recycle_frame(rotated);
					}
				} else {
